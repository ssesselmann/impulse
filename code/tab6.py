    ...
    html_tab6 = html.Div(id='tab6', children=[
        ...
        html.Div(id='tab6_text_div', children=[
            ...
            html.Div(id='manual', children=[
                ...
                html.H4('Automatic Temperature Calibration', style={'display': serial}),
                ...
                html.P("""
                Temperature calibration typically requires many hours of waiting for the detector temperature to stabilise between recordings. Impulse provides an easy-to-use automated calibration process. The idea is to start from a low temperature and let the detector slowly warm up.

                You can create a simple calibration chamber using an insulated cooler bag. Place the detector inside along with pre-chilled metal bars wrapped in a towel. Any dense, heat-conductive material will do. If you don't have gold bars handy, exercise dumbbells work well too — just put them in the freezer overnight and wrap them in a towel before placing them alongside your detector.
                """, style={'display': serial}),
                ...
                html.H5("Setting the Calibration Options", style={'display': serial}),
                html.P("""
                On Tab 1, you can set the number of calibration points and the temperature step (∆C˚) between each point.

                To modify the advanced parameters, open the `_settings.json` file directly:
                """, style={'display': serial}),
                ...
                html.H5("Step-by-Step Procedure", style={'display': serial}),
                html.Ol([
                    html.Li("Attach the temperature probe to the side of your detector (near the crystal) with masking tape."),
                    html.Li("Tape a calibration source (e.g., Cs-137) to the front of the detector."),
                    html.Li("Place the detector in the cooler bag with the chilled metal bars."),
                    html.Li("Allow the temperature to stabilise at the coldest point (e.g., 0°C)."),
                    html.Li("Estimate your calibration steps (e.g., from 0°C to 30°C in 10 steps of 3°C)."),
                    html.Li("Start the Automatic Calibration process and wait."),
                    html.Li("If needed, place the cooler bag near a heater to reach the final temperature."),
                    html.Li("The program will finish automatically when all calibration steps are complete."),
                    html.Li("Your spectrometer is now calibrated."),
                ], style={'display': serial}),
